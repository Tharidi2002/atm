...
CREATE DATABASE IF NOT EXISTS atm_security_db;
USE atm_security_db;
...
-- 3.1 ATM Machines - Main ATM
INSERT INTO `atm_machines` (`atm_code`, `location`, `sim_number`, `status`) 
VALUES 
    ('ATM-MAIN-01', 'Colombo 03', '0772032675', 'ACTIVE'),
    ('ATM-Z8B-01', 'Colombo 07 - Kollupitiya', '0714868100', 'ACTIVE'),
    ('ATM-Z8B-02', 'Colombo 05 - Havelock Town', '0714868101', 'ACTIVE'),
    ('ATM-Z8B-03', 'Galle - Main Street', '0714868102', 'ACTIVE'),
    ('ATM-Z8B-04', 'Kandy - City Center', '0714868103', 'ACTIVE');

-- 3.2 ATM Zones - Zone mapping for ATM-MAIN-01
INSERT INTO `atm_zones` (`atm_id`, `zone_number`, `zone_name`) 
SELECT 
    (SELECT id FROM atm_machines WHERE atm_code = 'ATM-MAIN-01'),
    zone_num,
    zone_name
FROM (
    SELECT 1 as zone_num, 'Main Entrance' as zone_name UNION ALL
    SELECT 2, 'Cash Counter Area' UNION ALL
    SELECT 3, 'ATM Lobby' UNION ALL
...
-- 3.3 ATM Zones - Zone mapping for Z8B ATMs
INSERT INTO `atm_zones` (`atm_id`, `zone_number`, `zone_name`) 
SELECT 
    (SELECT id FROM atm_machines WHERE atm_code = 'ATM-Z8B-01'),
    zone_num,
    zone_name
FROM (
    SELECT 1 as zone_num, 'Main Door' as zone_name UNION ALL
    SELECT 2, 'Cash Dispenser' UNION ALL
    SELECT 3, 'Card Reader' UNION ALL
...
INSERT INTO `atm_zones` (`atm_id`, `zone_number`, `zone_name`) 
SELECT 
    (SELECT id FROM atm_machines WHERE atm_code = 'ATM-Z8B-02'),
    zone_num,
    zone_name
FROM (
    SELECT 1 as zone_num, 'Main Door' as zone_name UNION ALL
...
INSERT INTO `atm_zones` (`atm_id`, `zone_number`, `zone_name`) 
SELECT 
    (SELECT id FROM atm_machines WHERE atm_code = 'ATM-Z8B-03'),
    zone_num,
    zone_name
FROM (
    SELECT 1 as zone_num, 'Main Door' as zone_name UNION ALL
...
INSERT INTO `atm_zones` (`atm_id`, `zone_number`, `zone_name`) 
SELECT 
    (SELECT id FROM atm_machines WHERE atm_code = 'ATM-Z8B-04'),
    zone_num,
    zone_name
FROM (
    SELECT 1 as zone_num, 'Main Door' as zone_name UNION ALL
...
-- 4.1 Z8B ATM Alerts (Zone 01)
INSERT INTO `alert_logs` (`atm_id`, `zone_number`, `zone_numbers`, `alert_type`, `raw_message`, `received_at`, `status`)
SELECT 
    (SELECT id FROM atm_machines WHERE atm_code = 'ATM-Z8B-01'),
    1, '01',
    'ZONE 01 ALARM!',
...
LEFT JOIN alert_logs al ON am.id = al.atm_id
GROUP BY am.id, am.atm_code, am.location;
...
GROUP BY am.atm_code, al.zone_number, al.zone_numbers
...
-- 6.1 Get Alerts by ATM
DELIMITER //
CREATE PROCEDURE `sp_get_atm_alerts`(IN p_atm_code VARCHAR(50))
BEGIN
    SELECT 
        al.*,
        am.atm_code,
...
-- 7.3 Check ATM machines
SELECT * FROM atm_machines;

-- 7.4 Check ATM zones
SELECT * FROM atm_zones;

-- 7.5 Check recent alerts with ATM details
SELECT 
    al.id,
    am.atm_code,
    am.location,
...
-- CREATE INDEX idx_alert_logs_atm_id ON alert_logs(atm_id);
